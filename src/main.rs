...
async fn main() {
    ...
    let shared = std::sync::Arc::new(parking_lot::Mutex::new(AppState {
        rooms: HashMap::new(),
        conns: HashMap::new(),
        txs: HashMap::new(),
        auth_service,
...
async fn websocket_handler(ws: WebSocketUpgrade, State(state): State<std::sync::Arc<parking_lot::Mutex<AppState>>>) -> axum::response::Response {
    ws.on_upgrade(|socket| handle_socket(socket, state))
}

async fn handle_socket(socket: WebSocket, state: std::sync::Arc<parking_lot::Mutex<AppState>>) {
...
async fn handle_client_msg(conn_id: Uuid, cmd: ClientMsg, state: &std::sync::Arc<parking_lot::Mutex<AppState>>) {
    match cmd {
...
// API handlers
async fn register(
    State(state): State<std::sync::Arc<parking_lot::Mutex<AppState>>>,
